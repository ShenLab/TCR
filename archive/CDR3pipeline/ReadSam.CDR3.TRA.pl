...
###########The code originally named as "Yaping.FindVDJ6_4.AssignProb_CDR3_2.pl" for Mouse TRA. Adapted for human TRA
...
##########update to find the indels and mismatchs of TRBV and TRBJ
##########Fix the problem that productive CDR3 sequences lose 
##########add V mapping quality score
######### add 3 genes TRAJ51, TRAJ60, and TRAJ55 in coordinates (v7)
#!/usr/bin/perl
#use Bio::DB::Bam;
use strict;
use warnings;
...
open(FinIgHpos,'TRA.v37.YapingCoordinates_VDJonly3.txt');
...
close(FinIgHpos);
###the whole genome coordinates should add 22090056 
...
#print "$i\t$values[13]\n";
	...
	#print $values[9],"\n";
	if ($#values > 3) { # remove SQ lines & sequences with "N"
		if($values[9]=~/^(N+)(.*[^N])(N+)$/){
                $values[9]=$2;
		}
		$startT[0] = $values[3];
		...
		for (my $i=0;$i<=$#startRM;$i++) {
				for (my $j=0; $j<$#IgHanno; $j++){
					if (($startTM[$i]>=$IgHanno[$j][2]&&$startTM[$i]<=$IgHanno[$j][3])||($endTM[$i]>=$IgHanno[$j][2]&&$endTM[$i]<=$IgHanno[$j][3])){
						if (substr($IgHanno[$j][1],0,4) eq "TRAV"){  #$IgHanno[][7] is the DNA sequence for the gene
							push(@VPos,$values[1]);		
							push(@VPos, $startRM[$i]);
							push(@VPos, $endRM[$i]);
						...
						elsif (substr($IgHanno[$j][1],0,4) eq "TRAJ"){
							push(@JPos,$values[1]);
							push(@JPos, $startRM[$i]);
							push(@JPos, $endRM[$i]);
		...
		if ($indxP==0){
			...
			if ($#cdrV>=2 && $#cdrJ>=2){
				if ($cdrV[0]==16 && $cdrJ[0]==16){
					...
					if ($values_ex[1]==16){
						#print length($values_ex[9]),"\t",$cdrStart,"\t",$cdrEnd,"\n";
